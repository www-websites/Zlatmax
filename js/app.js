(() => {
    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                if (bodyLockStatus && e.target.closest(".icon-menu")) {
                    ...
                    document.querySelector(".categories-show") ? document.querySelector(".categories-show").classList.remove("categories-show") : null;
        ...
        function rangeInit() {
            const priceRangeItems = document.querySelectorAll("[data-price-range-item]");
            if (priceRangeItems.length) priceRangeItems.forEach((priceRangeItem => {
                const priceRangeFrom = priceRangeItem.querySelector("[data-range-price-from]");
                const priceRangeTo = priceRangeItem.querySelector("[data-range-price-to]");
                const priceRange = priceRangeItem.querySelector("[data-range-price]");
                nouislider.create(priceRange, {
                    start: [ Number(priceRangeFrom.value), Number(priceRangeTo.value) ],
                    connect: true,
                    tooltips: [ true, true ],
                    range: {
                        min: [ Number(priceRangeFrom.dataset.rangePriceFrom) ],
                        max: [ Number(priceRangeTo.dataset.rangePriceTo) ]
                    },
                    format: {
                        from: function(value) {
                ...
                priceRange.noUiSlider.on("update", (function(values, handle) {
                    const priceRangeLower = priceRangeItem.querySelector(".noUi-handle-lower");
                    const priceRangeMin = priceRangeLower.getAttribute("aria-valuenow");
                    priceRangeFrom.value = parseInt(priceRangeMin);
                    const priceRangeUpper = priceRangeItem.querySelector(".noUi-handle-upper");
                    const priceRangeMax = priceRangeUpper.getAttribute("aria-valuenow");
                    priceRangeTo.value = parseInt(priceRangeMax);
                }));
                priceRangeFrom.addEventListener("change", setPriceValues);
                priceRangeTo.addEventListener("change", setPriceValues);
                function setPriceValues() {
                    let priceStartValue;
                    let priceEndValue;
                    if ("" != priceRangeFrom.value) priceStartValue = priceRangeFrom.value;
                    if ("" != priceRangeTo.value) priceEndValue = priceRangeTo.value;
                    priceRange.noUiSlider.set([ priceStartValue, priceEndValue ]);
                }
            }));
            ...
            if (rangeItems.length) rangeItems.forEach((rangeItem => {
                ...
                const rangeTo = rangeItem.querySelector("[data-range-to]");
                const range = rangeItem.querySelector("[data-range]");
                nouislider.create(range, {
                    start: [ Number(rangeFrom.value), Number(rangeTo.value) ],
                    connect: true,
                    range: {
                        min: [ Number(rangeFrom.dataset.rangeFrom) ],
                        max: [ Number(rangeTo.dataset.rangeTo) ]
                    }
                });
                range.noUiSlider.on("update", (function(values, handle) {
                    const rangeLower = rangeItem.querySelector(".noUi-handle-lower");
                    const rangeMin = rangeLower.getAttribute("aria-valuenow");
                    rangeFrom.value = parseInt(rangeMin);
                    const rangeUpper = rangeItem.querySelector(".noUi-handle-upper");
                    const rangeMax = rangeUpper.getAttribute("aria-valuenow");
                    rangeTo.value = parseInt(rangeMax);
                }));
                rangeFrom.addEventListener("change", setRangeValues);
                rangeTo.addEventListener("change", setRangeValues);
                function setRangeValues() {
                    let rangeStartValue;
                    let rangeEndValue;
                    if ("" != rangeFrom.value) rangeStartValue = rangeFrom.value;
                    if ("" != rangeTo.value) rangeEndValue = rangeTo.value;
                    range.noUiSlider.set([ rangeStartValue, rangeEndValue ]);
                }
            }));
        }
        rangeInit();
        ...
        function initSliders() {
            if (document.querySelector(".intro__slider")) new core(".intro__slider", {
                ...
                on: {
                    init: function(swiper) {
                        const allSlider = document.querySelector(".fraction-intro__all");
                        const allSlidesItems = document.querySelectorAll(".slide-intro:not(.swiper-slide-duplicate)");
                        allSlider.innerHTML = allSlidesItems.length < 10 ? `0${allSlidesItems.length}` : allSlidesItems.length;
                    },
                    slideChange: function(swiper) {
                        const currentSlide = document.querySelector(".fraction-intro__current");
            ...
            if (document.querySelector(".products-dark__slider")) new core(".products-dark__slider", {
                modules: [ Navigation, Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
                ...
                speed: 800,
                pagination: {
                    el: ".products-dark-swiper-pagination",
                    clickable: true
                },
                breakpoints: {
                    0: {
            ...
            if (document.querySelector(".images-product__slider")) {
                const thumbsSwiper = new core(".product-thumbs", {
                    modules: [ Thumb ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 4,
        ...
        function documentActions(e) {
            ...
            if (headerCatalog) {
                if (targetElement.closest("button.catalog-header__link-category")) {
                    const catalogCategory = targetElement;
                    const catalogCategoryIndex = catalogCategory.getAttribute("data-menu-category");
                    const catalogSubMenu = document.querySelector(`[data-sub-menu="${catalogCategoryIndex}"]`);
                    function headerCatalogRemoveAll() {
                        document.querySelector(".link-category-active") ? document.querySelector(".link-category-active").classList.remove("link-category-active") : null;
                        document.querySelector(".sub-menu-open") ? document.querySelector(".sub-menu-open").classList.remove("sub-menu-open") : null;
                    }
                    if (catalogCategory === document.querySelector(".link-category-active")) headerCatalogRemoveAll(); else {
                        headerCatalogRemoveAll();
                        catalogCategory.classList.add("link-category-active");
                        if (catalogCategory.classList.contains("link-category-active")) catalogSubMenu.classList.add("sub-menu-open");
                        document.documentElement.classList.add("catalog-open");
                    }
                }
                if (targetElement.closest(".link-catalog")) {
                    const catalogCategories = document.querySelector(".catalog-header__categories");
                    catalogCategories.classList.add("categories-show");
                }
                if (targetElement.closest(".catalog-header__back")) document.querySelector(".categories-show") ? document.querySelector(".categories-show").classList.remove("categories-show") : null;
                if (targetElement.closest(".sub-menu-catalog__back")) document.querySelector(".sub-menu-open") ? document.querySelector(".sub-menu-open").classList.remove("sub-menu-open") : null;
            }
        }
        const quantityButtons = document.querySelectorAll(".quantity-product__button");
        if (quantityButtons.length) {
            const productPriceElement = document.querySelector(".buy-product__price");
            const productStaticPrice = productPriceElement.innerHTML;
            quantityButtons.forEach((quantityButton => {
                quantityButton.addEventListener("click", (function(e) {
                    const quantityButtonDirection = this.dataset.direction;
                    const quantityAmount = this.parentElement.querySelector(".quantity-product__amount");
                    const currentValue = +quantityAmount.value;
                    let newValue;
                    if ("more" === quantityButtonDirection) newValue = currentValue + 1; else if ("less" === quantityButtonDirection) newValue = currentValue - 1;
                    if (newValue > 0) quantityAmount.value = newValue;
                    if ("more" === quantityButtonDirection) if (productPriceElement) {
                        const productActualPrice = productPriceElement.innerHTML;
                        productPriceElement.innerHTML = +productActualPrice + +productStaticPrice;
                        console.log(productPriceElement);
                    }
                    if ("less" === quantityButtonDirection) if (quantityAmount.value > 0) {
                        const productActualPrice = productPriceElement.innerHTML;
                        if (productActualPrice !== productStaticPrice) {
                            productPriceElement.innerHTML = +productActualPrice - +productStaticPrice;
                            console.log(productPriceElement);
                        }
                    }
